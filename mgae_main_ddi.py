...
def train(model, predictor, x, data, split_edge, optimizer, args):
    ...
    adj, _, pos_train_edge = random_edge_mask(args, split_edge, x.device, x.size(0))
    adj = adj.to(x.device)
...
@torch.no_grad()
def test(model, predictor, x, data, split_edge, evaluator, batch_size):
    model.eval()
    predictor.eval()
...
def main():
    ...
    save_path_emb = 'weight/mgaev2-' + args.use_sage + '_{}_{}'.format(args.dataset,
                                                                         args.decoder_mask) + '_{}'.format(
        args.num_layers) + '_hidd{}-{}-{}-{}-{}-{}'.format(args.hidden_channels, args.keep_prob, args.decode_layers,
                                                     args.decode_channels, args.lr, args.dropout) + '_emb.pth'
    save_path_predictor = 'weight/mgaev2udict' + args.use_sage + '_{}_{}'.format(args.dataset,
                                                                          args.decoder_mask) + '_{}'.format(
        args.num_layers) + '_hidd{}-{}-{}-{}-{}-{}'.format(args.hidden_channels, args.keep_prob, args.decode_layers,
    ...
    print('Start training with mask ratio={} # optimization edges={}'.format(args.keep_prob,
                                                                             int(args.keep_prob * split_edge['train']['edge'].shape[0])))
    ...
    if args.use_sage == 'SAGE':
        ...
        model = GIN(args.hidden_channels, args.hidden_channels,
                     args.hidden_channels, args.num_layers,
                     args.dropout).to(device)
    else:
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            t1 = time.time()
            loss = train(model, predictor, emb.weight, data, split_edge,
                         optimizer, args)
            t2 = time.time()

            results = test(model, predictor, emb.weight, data, split_edge,
